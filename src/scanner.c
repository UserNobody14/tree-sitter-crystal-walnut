...
bool tree_sitter_cwal_external_scanner_scan(void *payload, TSLexer *lexer, const bool *valid_symbols) {
            ...
            if (lexer->lookahead == '\\') {
                ...
                if (is_bytes(delimiter)) {
                    ...
                    if (lexer->lookahead == 'N' || lexer->lookahead == 'u' || lexer->lookahead == 'U') {
                        ...
                        // https://docs.cwal.org/3/reference/lexical_analysis.html#string-and-bytes-literals
                        advance(lexer);
                    } else {
                        lexer->result_symbol = STRING_CONTENT;
                        return has_content;
...
unsigned tree_sitter_cwal_external_scanner_serialize(void *payload, char *buffer) {
    Scanner *scanner = (Scanner *)payload;
...
void tree_sitter_cwal_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
    Scanner *scanner = (Scanner *)payload;
...
void *tree_sitter_cwal_external_scanner_create() {
    ...
    tree_sitter_cwal_external_scanner_deserialize(scanner, NULL, 0);
    return scanner;
}

void tree_sitter_cwal_external_scanner_destroy(void *payload) {
    Scanner *scanner = (Scanner *)payload;
    array_delete(&scanner->indents);
    array_delete(&scanner->delimiters);
    free(scanner);
